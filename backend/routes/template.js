  ...
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(rows) || rows.length === 0) {
        return res
          .status(400)
          .json({ success: false, error: "Array of categories required." });
      }
      let upserted = 0;
  ...
  async (req, res) => {
    try {
      ...
      if (!categoryCode) {
        return res
          .status(400)
          .json({ success: false, error: "categoryCode is required." });
      }
      ...
      if (err.code === 11000) {
        return res
          .status(409)
          .json({ success: false, error: "Category code already exists." });
      }
      res.status(500).json({ success: false, error: err.message });
    }
  },
  ...
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(rows) || rows.length === 0) {
        return res
          .status(400)
          .json({ success: false, error: "Array of stores required." });
      }
      let upserted = 0;
      for (const row of rows) {
  ...
  async (req, res) => {
    try {
      ...
      if (!storeCode) {
        return res
          .status(400)
          .json({ success: false, error: "storeCode is required." });
      }
      ...
      if (err.code === 11000) {
        return res
          .status(409)
          .json({ success: false, error: "Store code already exists." });
      }
      res.status(500).json({ success: false, error: err.message });
    }
  },
