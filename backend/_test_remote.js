      ...
      function renderTransactions(txs) {
        ...
        function render() {
          ...
          let html =
            ...
            " transactions</div>";
          html +=
            '<div style="overflow-x:auto"><table id="txTable"><thead><tr>';
          html +=
      ...
      async function loadUsers() {
        ...
        try {
          ...
          users.forEach((u) => {
            ...
            html += '<div class="user-avatar ' + avClass + '">' + esc(initials) + '</div>';
            html += '<div class="user-info">';
            html += '<div class="name">' + esc(u.username) + ' <span class="role-badge ' + u.role + '">' + u.role.toUpperCase() + '</span></div>';
            html += '<div class="meta">Joined ' + dateStr + '</div>';
            html += '</div>';
            html += '</div>';
            html += '<div class="user-card-actions">';
            html += '<button class="btn-sm" style="background:#e3f2fd;color:#1565c0" onclick="openPinModal(\'' + esc(u.username).replace(/'/g, "\\'") + '\')">&#128273; Reset PIN</button>';
            html += '<button class="btn-sm" style="background:#fff3e0;color:#e65100" onclick="deleteUserDataOnly(\'' + esc(u.username).replace(/'/g, "\\'") + '\')">&#128451; Archive Data</button>';
            if (u.role !== 'admin') {
              html += '<button class="btn-sm" style="background:#fce4ec;color:#c62828" onclick="deleteUserOnly(\'' + esc(u.username).replace(/'/g, "\\'") + '\')">&#128100; Delete User</button>';
              html += '<button class="btn-sm" style="background:#d32f2f;color:#fff" onclick="deleteUserAndData(\'' + esc(u.username).replace(/'/g, "\\'") + '\')">&#128465; Delete User + Data</button>';
            }
            html += '</div>';
            html += '</div>';
          });
      ...
      function goToPinStep2() {
        ...
        document.getElementById('pinResultMsg').innerHTML = '';
        setTimeout(function() { document.getElementById('pinNewInput').focus(); }, 100);
      }
      ...
      async function savePinReset() {
        ...
        msg.innerHTML = '';
        if (!pin || pin.length < 4) {
          msg.innerHTML = '<span style="color:#c62828">&#9888; PIN must be at least 4 digits.</span>';
          document.getElementById('pinNewInput').focus();
          return;
        }
        if (pin !== confirmPin) {
          msg.innerHTML = '<span style="color:#c62828">&#9888; PINs do not match. Please re-enter.</span>';
        ...
        try {
          ...
          msg.innerHTML = '<span style="color:#2e7d32">&#10004; PIN reset successfully for <strong>' + esc(pinResetTargetUser) + '</strong>!</span>';
          setTimeout(function() { closePinModal(); }, 1500);
        } catch (err) {
          msg.innerHTML = '<span style="color:#c62828">&#9888; ' + esc(err.message) + '</span>';
          document.getElementById('pinSaveBtn').disabled = false;
        }
      }
